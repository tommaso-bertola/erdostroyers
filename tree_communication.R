    ...
    if(nrow(packets)!=0) {
      for(j in 1:nrow(packets)) {
        if(nodes_queue[[packets[j,"curr"]]][1] == packets[j, "id"]) {
          ...
          #following line is the only relevant line for the different assigning strategy
          #packets[j, "curr"] = closest_neighbors[sample(length(closest_neighbors), 1)]
          ##################start of differing part according to closest neighbors
          ...
          fa = ifelse(length(nodes_queue[[packets[j,"curr"]]])>0, length(nodes_queue[[packets[j,"curr"]]])>0, 1)
          kab = xab*fa^(-gamma)
          xba = runif(1, min=0, max=1) #change to one to switch off noise
          fb = ifelse(length(nodes_queue[[closest_neighbor]])>0, length(nodes_queue[[packets[j,"curr"]]])>0, 1)
          kba = xba*fb^(-gamma)
          qab = sqrt(xab*kab*xba*kba)
          hop = runif(1, 0, 1)
          if(hop<qab){
            packets[j, "curr"] = closest_neighbors[sample(length(closest_neighbors), 1)]
            nodes_queue[[packets[j, "curr"]]] = c(nodes_queue[[packets[j, "curr"]]], packets[j, "id"])
          }
          ##################end of differing part according to closest neighbors
